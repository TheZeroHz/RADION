...
BatteryManager::BatteryManager(int adcPin, double conversionFactor, int reads) {
  _reads = reads;
  _conversionFactor = conversionFactor;
  _adcPin = adcPin;
}

BatteryManager::BatteryManager(int adcPin, double conversionFactor) {
  _reads = DEFAULT_READS;
  _conversionFactor = conversionFactor;
  _adcPin = adcPin;
}

BatteryManager::BatteryManager(int adcPin) {
  _reads = DEFAULT_READS;
...
double BatteryManager::getBatteryVolts() {
  int readValue = _avgAnalogRead(_adcPin, _reads);
  return _analogReadToVolts(readValue);
}

int BatteryManager::getBatteryChargeLevel(bool useConversionTable) {
  int readValue = _avgAnalogRead(_adcPin, _reads);
...
void BatteryManager::_initConversionTable() {
...
int BatteryManager::_avgAnalogRead(int pinNumber, int reads) {
  int totalValue = 0;
...
int BatteryManager::_calculateChargeLevel(double volts) {
  if (volts <= 3.700) {
    return (20 * volts) - 64;
  }
...
int BatteryManager::_getChargeLevelFromConversionTable(double volts) {
...
double BatteryManager::_analogReadToVolts(int readValue) {
